...
| EDIT-13 | Core  | E3               | Node prose write/read results expose the live numeric `wordCount`; every persisted scalar carries an exact seed/Yjs semantic hash plus local revision or Server sequence basis, and a requested minimum length is verified from persisted content rather than model self-report.                                                                          |

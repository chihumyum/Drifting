...
| TOOL-11 | Core  | E3               | A compound mutation keeps the installed `revise_object`/`write_object` verbs available across later iterations; no previously installed verb degrades into `UNKNOWN_TOOL`.                                                                                                                                                                                                 |
...
| TOOL-14 | Core  | E1+E3            | The ordinary provider surface exposes only `browse_project`, `read_object`, `search_work`, `revise_object`, `write_object` and `delete_object` plus task controls. Their public schemas accept author-facing targets/fields and contain no path, extension, JSON document, Yjs, SQLite, revision or file-operation vocabulary.                                             |
...
| READ-09 | Core  | E2               | A narrow `search_work` inside one authored object or collection is literal, reports an exact occurrence count, and represents zero matches explicitly.                                                                                                              |

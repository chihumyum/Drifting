...
- `word_count_basis_server_seq`: a dormant derived-provenance slot. The current
  local build leaves it null, and SyncEngine never transports it.

A missing basis means an incomplete projection. UI, project summaries,
...
- Manual editor persistence captures one stable Yjs base and commits
  `node_content`, outline, count, basis and the authored change-set journal in
  one SQLite transaction guarded by the captured Yjs revision.
...
## SyncEngine remote-apply boundary

Remote objects carry authoritative Yjs updates or full Yjs state, never a
trusted word-count scalar. After the deterministic reducer applies prose, the
local materializer recomputes count, semantic hash and local revision from that
device's resulting Yjs state. Checkpoints may carry the authored Yjs state, but
word-count columns and their basis remain rebuildable projections.

The retired entity/Yjs HTTP push-pull protocol is not a compatibility surface.
Future providers, including a hosted provider, exchange the same opaque
immutable SyncEngine objects and cannot inject `word_count_basis_server_seq`
through domain CRUD.

Project totals are chapter-only. Drift nodes retain their own exact count but do
not contribute to project targets, Writing History, or project aggregate
projections. The local Writing History store upgrades to version 2 by clearing
the incompatible v1 total snapshots while preserving author-owned project and
daily goals. Version 3 stores `{ startTotal, latestTotal }` for each day. Its v2
...
hashing, seed versus revision-backed projections, shelf reconciliation,
first-observation daily baselines, Agent-created prose, and order-independent
concurrent Yjs merge materialization.
...
code, but physical-device foreground/background lifecycle, touch, safe-area,
IME and visual presentation remain manual acceptance. Cross-device convergence
requires its own SyncEngine provider gate. Build, typecheck, Simulator, or a

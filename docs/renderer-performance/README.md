...
`--graph-interactions` 在隔离合成数据与新构建应用中，通过实际图谱组件完成写入，
再正常退出、重启并读取 SQLite 核对。夹具包含 55 个正文节点、三条故事线、两个
Drift 和 500 条既有关系；两份从空目录独立生成的数据库语义摘要一致。

未放置章节落在叙事坐标 2.5，已放置章节移动至 4.25；两者从主线 A 移到目标主线 B，
保留独立副线 C，书序仍为 48 / 47。旧主线 A 的成员关系按既有移动规则移除，不会
自动变成另一条副线。取消拖动后 SQLite 结构完全不变。Story Graph 新建两条有向
Drift 关系，覆盖 Drift 作为源和目标，并操作方向交换控件；Super Element 新建
元素到 Drift 的关系。Drift 拖到叙事轴生成坐标 6.75、空标签的绑定标记，原 Drift
正文与叙事坐标保留。关闭面板只隐藏特殊边几何，三条关系仍持久存在。

PointerEvent 驱动限定在全屏图谱内读取目标轨道，避免误用仍在挂载的底部时间线。
源卡片和目标行先滚动至可见位置，起点和落点必须通过 DOM hit test；重叠卡片从
露出的区域抓取，精确保留实际水平偏移。关系创建通过 Shift 配对、body portal
中的类型选择及确认按钮；标记使用 DragEvent。数据库查询仅用于观测，未绕过失败
的界面操作直接补写数据。

`acceptance/f5-graph-interactions-native.json` 记录新构建
`Drifting Acceptance de78b2264590.app`：首次组合 57 项、重启 5 项检查通过，
其中图谱首次 7 项与重启 4 项专项检查通过；既有编辑器、Copilot、Agent 面板及
会话历史组合回归一并通过。两次正常 Cmd+Q 均以 exit 0 退出，未捕获异常为 0。
二进制 SHA-256 为
`610756bb0eb8d8258265f3d8a9217532f39e5d4d9f3aa4e16b9de34ef70da571`。

独立只读 SQLite 进程比较完整结构前后快照，只允许两次叙事坐标/主线变更、三条
新关系和一个标记；其他节点、书序、副线、既有关系与标记保持。正文仅控制组第一章
增加保存标记，其余 54 个正文哈希不变。SQLite 完整性与外键检查通过。普通 CI
校验已捕获报告，并以缺失副线、错误坐标/方向/标记、失败取消/恢复、其他节点变更等
负向样本验证证据契约；不会把历史报告重新解释为当前设备性能测量。

```bash
pnpm perf:renderer:native --graph-interactions
pnpm perf:renderer:native --graph-interactions --check
```

全量测试 2,674 项通过、1 项原有跳过；类型、CI 契约、公开边界与 Agent 能力检查
通过。Lint 为 0 错误、74 条既有警告（包含主工作区内的旧嵌套检出）。正常本地模式
生产构建输出 32 个 JS chunk，不含原生验收入口或合成数据标记。
`acceptance/f5-graph-interactions.json` 已从当前源码重新生成，并通过当前指纹与
浏览器确定性契约检查；它记录浏览器回归，原生持久写入由独立 native 报告证明。

这是原生 WebKit 中的合成 PointerEvent/DragEvent、Tiptap 命令与 DOM 点击验收，
不是物理鼠标、触摸、IME、移动设备或大图性能预算验收。F5-03 与整个 F5 保持 partial /
in_progress；尚未执行的输入、设备与性能门槛继续保留。
...
固定设备性能预算。本批结束时尚待验收的真实关系/标记持久化与正常退出/重启，已由后续 F5i 覆盖。
F5-01–04 继续保持 partial。

该独立批次完整测试 2,542 项通过、1 项原有跳过；类型、Lint、CI 契约、公开边界、
Agent 能力和当前源码浏览器确定性检查通过。隔离检出的 Lint 为 0 错误、33 条既有
警告；当时主工作区还扫描了嵌套旧检出，总计 74 条警告。
正常本地模式构建输出 32 个 JS chunk，不含本批夹具、计数器或原生验收入口标记。

## F4k：发送准备与已提交回合的独立归属

新增的相同 22 项 store 检查在基线 `c9a21018` 上有 20 项失败：等待记忆读取时
新建/载入会话、换项目、删除或停止后，旧发送仍取走拒绝记录并调用模型启动；旧准备
迟迟不释放全局 `starting`。运行时启动确认未返回时导航，又会丢失已提交后台回合的
运行指针。`agent:send:acceptance` 从隔离基线检出运行相同测试，再运行当前四组
测试，生成 `acceptance/f4-send-preparation.json`；44 项当前检查均通过。

`chat-start-owner.ts` 现在拥有一个前台准备意图及尚待启动确认的回合。失效立即释放
准备占用并清理该回合的临时路由；旧 Promise 的 finally 无权释放新的意图。每次
记忆读取后重新检查归属，失效后不启动下一步、不取走拒绝记录、不调用模型启动。
为继续对话而分叉会话的请求若迟到失败，失效后的错误也不会写回旧会话。
`chat-send-preparation.ts` 负责可失效的上下文准备和纯拒绝说明格式化；store 保留
作者消息、会话持久化调用和运行时命令的接线，行数从 1,295 降至 1,240。

提交边界之后，同项目新建/载入会话保留后台回合；停止、换项目、删除或清除历史若
先于成功启动确认，则在确认返回后再次发送该 turn 的取消请求。迟到失败只回到其
所属会话，不能改动当前会话或解锁新的准备。同步启动异常也通过同一失败清理路径。
“工具后停止”保留原策略，不因此改成即时取消。销毁拒绝后续启动，清理只执行一次。
已接受的作者文本仍进入原会话，运行时上下文不进入作者可见消息；provider-neutral
协议、Yjs 正文及 SQLite schema 均未改变。

```bash
pnpm agent:send:acceptance
pnpm agent:send:acceptance --check
```

本批完整测试 2,567 项通过、1 项原有跳过；隔离 Lint 为 0 错误、33 条既有警告。
类型、CI 契约、公开边界与 Agent 能力检查通过。正常本地模式构建仍为 32 个 JS
chunk，未混入验收夹具或计数器。`acceptance/f4-send-preparation-browser.json`
通过当前源码指纹和确定性约束，验证既有输入、显示、图谱和工作区浏览器回归保持。
仓库/记忆/transport 使用合成端口，启动取消检查证明请求和 store 归属，不证明真实
...
## F2 — Shared entity-link target existence lookup

[Entity-link target state](link-target-state.md) is now a separate read function
using the renderer's existing shared ID indexes. The editor configuration hook
reads the current store at each resolution and delegates existence/trash semantics
to that function. It preserves alive-row precedence, untracked-kind navigation,
immutable-snapshot replacement and the existing decoration refresh lifecycle.

`acceptance/f2-link-target-state-browser.json` was generated by the ordinary
production headless harness. Nine size/consumer profiles return the same states
as the prior linear reference. In the deliberate last/missing-ID case with
four 5,000-row collections and 20 consumers making 500 lookups each, record reads
fall from 50,000,000 to 20,000 for initial shared index construction; the repeated
query pass performs zero collection reads. First construction remains linear,
and neither map lookups nor full-document decoration traversal disappear.

Twenty-five browser checks include actual visible/hidden Tiptap editors mounting
the product hook through deletion, restoration and A→B→A project replacement.
Editor instances and prose remain unchanged. Six unit tests cover all four
tracked kinds and snapshot reuse/replacement. Ordinary deterministic CI now
requires the new section, with mutation checks rejecting omitted evidence,
repeated scans, an unbounded index and failed behavior checks.

All six required repository checks and the architecture command passed. The
full suite has 2,796 passing tests and one existing skip across 421 files; lint
has zero errors and 30 existing warnings. The normal production build has
42 JavaScript assets and excludes all acceptance probes. The generated report
retains its actual pre-commit SHA and measured source fingerprint. Native

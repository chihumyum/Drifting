...
const measured = 'src/renderer/services/workspace-projection-element-reads.acceptance.test.ts';
const suites = ['src/renderer/services/workspace-projection-elements.integration.test.ts',
  'src/renderer/services/workspace-projection-coverage.integration.test.ts'];
const output = path.join(root, 'docs/renderer-performance/acceptance/f6-element-reads.json');
const hash = value => createHash('sha256').update(value).digest('hex');
const fingerprint = () => workspaceEvidenceFingerprint(root);
const git = (...args) => execFileSync('git', args, { cwd: root, encoding: 'utf8' }).trim();
function validate(report) {
  assert.equal(report.kind, 'workspace_element_reads_acceptance'); assert.equal(report.status, 'passed');
  assert.deepEqual(report.suites, [...suites, measured]); assert.equal(report.tests.length, 55);
  assert(report.tests.every(test => test.status === 'passed' && test.durationMs >= 0));
  assert.equal(new Set(report.tests.map(test => test.name)).size, 55);
  ...
  for (const scenarios of [report.baseline.reads, report.reads]) {
    ...
    for (const scenario of scenarios) {
      ...
      for (const sample of scenario.samples) {
        for (const field of ['queries', 'rows', 'serializedBytes', 'elementBodyRows']) assert(Number.isSafeInteger(sample[field]) && sample[field] > 0);
  ...
  for (let index = 0; index < report.reads.length; index++) {
    ...
    assert(baseline.samples.every(sample => sample.elementBodyRows === baseline.entities + 1 && sample.queries === 5));
    assert(current.samples.every(sample => sample.elementBodyRows === 1 && sample.queries === 5 && sample.rows === current.entities + 5));
  ...
  assert.deepEqual(report.acceptance, { productionCapture: true, sqlite: 'temporary-WAL-FULL', fullProjectionOracle: true,
    unchangedElementObjects: true, fullFallback: true, nativeRenderer: 'not-run', fullAppPerformance: 'not-measured' });
}
if (process.argv.includes('--check')) {
  const report = JSON.parse(readFileSync(output, 'utf8')); validate(report);
  assert.equal(report.source.fingerprint, fingerprint(), 'Element read evidence is stale.');
  console.log('Element read evidence matches source: 55 checks and identical before/after SQLite measurement probes.');
} else {
  const temporary = mkdtempSync(path.join(tmpdir(), 'drifting-workspace-elements-'));
  ...
  function run(directory, files, name, measure = false) {
    ...
    const result = spawnSync('pnpm', ['exec', 'vitest', 'run', ...files, '--reporter=json', `--outputFile=${resultFile}`],
      { cwd: directory, encoding: 'utf8', env: { ...process.env, ...(measure ? { DRIFTING_ELEMENT_READ_COUNTERS: countersFile } : {}) } });
  ...
  try {
    ...
    const report = { kind: 'workspace_element_reads_acceptance', status: 'passed', generatedAt: new Date().toISOString(),
      ...
      acceptance: { productionCapture: true, sqlite: 'temporary-WAL-FULL', fullProjectionOracle: true, unchangedElementObjects: true,
      ...
      limitations: [
        ...
        'Up to 128 covered live element identities read bodies individually; SQLite still scans/sorts all live element IDs and the renderer merges an O(N) ID array. Queries remain five and row count increases by one.',
        ...
        'Element order is updatedAt descending with SQLite rowid ascending for equal timestamps, shared by full, category and skinny reads. Category dependency changes are captured in the same read transaction.',
      ] };
    validate(report); writeFileSync(output, JSON.stringify(report, null, 2) + '\n');
    console.log('Element read acceptance passed: 55 checks; 65/1025 element bodies reduced to one per edit.');
  } finally {

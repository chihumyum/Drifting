...
import { validateTranscriptSummary } from './transcript-summary-contract.mjs';
const root = fileURLToPath(new URL('..', import.meta.url)); process.chdir(root);
const baselineCommit = '44bdb498043d17991fe4e4d3528afe009d34d932';
const output = process.argv.find(arg => arg.startsWith('--output='))?.slice(9) ?? 'docs/renderer-performance/acceptance/f4-transcript-summary.json';
const common = ['scripts/renderer-transcript-summary.tsx', 'scripts/renderer-transcript-summary.html', 'scripts/run-renderer-transcript-summary.mjs', 'scripts/transcript-summary-contract.mjs'];
...
if (process.argv.includes('--check')) {
  const report = JSON.parse(readFileSync(output, 'utf8')); validateTranscriptSummary(report);
  if (!process.argv.includes('--historical')) assert.equal(report.after.fingerprint, fingerprint(root), 'Transcript summary evidence is stale.');
  console.log(process.argv.includes('--historical') ? 'Historical transcript summary evidence passed; current source was not asserted.' : 'Transcript summary evidence matches current source; native and whole-app budgets remain open.');
} else {
  const temporary = mkdtempSync(path.join(tmpdir(), 'drifting-transcript-summary-acceptance-')); const baseline = path.join(temporary, 'baseline'); let added = false;
  ...
  async function measure(directory, name) {
    ...
    try {
      ...
      await build({ root: directory, configFile: false, envDir: false, logLevel: 'warn', plugins: [{ name: 'transcript-summary-observation', enforce: 'pre',
        transform(code, id) {
          if (id.endsWith('/domain/agent-chat-transcript.ts')) {
            ...
            code = code.replace(anchor, anchor + '\n    globalThis.__TRANSCRIPT_SUMMARY_WORK__.arrays++; globalThis.__TRANSCRIPT_SUMMARY_WORK__.flattenedRows += this.length;');
            const leaf = 'if (depth === 0) visit(tree as readonly AgentChatMessage[], start);';
            if (code.includes('forEachLeaf(')) { assert.equal(code.split(leaf).length, 2); code = code.replace(leaf, 'if (depth === 0) { globalThis.__TRANSCRIPT_SUMMARY_WORK__.leaves++; visit(tree as readonly AgentChatMessage[], start); }'); }
          }
          if (id.endsWith('/features/agent/useAgentMessageBlocks.ts')) {
            const anchor = 'previous.messages[offset]'; assert(code.includes(anchor));
            code = code.replaceAll(anchor, '(globalThis.__TRANSCRIPT_SUMMARY_WORK__.comparisons++, previous.messages[offset])');
          }
          if (id.endsWith('/features/agent/desktop/DesktopAgentTranscript.tsx')) {
            const anchor = 'for (const m of messages) {';
            if (name === 'before') { assert.equal(code.split(anchor).length, 2); code = code.replace(anchor, anchor + ' globalThis.__TRANSCRIPT_SUMMARY_WORK__.usageCandidates++;'); }
            else assert(code.includes('useAgentTranscriptSummary(messageBlocks).usage'));
          }
          if (id.endsWith('/workspace/mobile-agent-model.ts')) {
            const anchor = 'for (const message of messages) {'; assert.equal(code.split(anchor).length, 2);
            code = code.replace(anchor, anchor + ' globalThis.__TRANSCRIPT_SUMMARY_WORK__.evidenceCandidates++;');
          }
          if (id.endsWith('/features/agent/agent-transcript-summary.ts')) {
            for (const [anchor, counter] of [['for (const message of block.messages) {', 'classifiedRows'], ['for (const message of content.usage) {', 'usageCandidates'], ['for (let index = 0; index < blocks.length; index++) {', 'summaryBlocks']]) {
              assert.equal(code.split(anchor).length, 2); code = code.replace(anchor, anchor + ` globalThis.__TRANSCRIPT_SUMMARY_WORK__.${counter}++;`);
            }
          }
      ...
      resolve: { alias: { '@': path.join(directory, 'src') } }, build: { outDir: dist, emptyOutDir: true, rollupOptions: { input: path.join(directory, 'scripts/renderer-transcript-summary.html') } } });
      ...
      const loaded = client.Page.loadEventFired(); await client.Page.navigate({ url: `http://127.0.0.1:${server.httpServer.address().port}/scripts/renderer-transcript-summary.html` }); await loaded;
      const result = await client.Runtime.evaluate({ expression: 'globalThis.__TRANSCRIPT_SUMMARY__.run()', awaitPromise: true, returnByValue: true });
  ...
  try {
    const suites = ['src/renderer/features/agent/agent-transcript-summary.test.ts', 'src/renderer/features/agent/useAgentMessageBlocks.test.ts', 'src/renderer/shells/mobile/workspace/mobile-agent-model.test.ts'];
    ...
    const report = { kind: 'agent_transcript_summary', status: 'passed', generatedAt: new Date().toISOString(), before, after, suites, tests,

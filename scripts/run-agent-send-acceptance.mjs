...
const integration = 'src/renderer/store/agent-chat-preparation.integration.test.ts';
const suites = [integration, 'src/renderer/lib/agent/runtime/chat-start-owner.test.ts',
  'src/renderer/store/agent-chat-store.test.ts', 'src/renderer/store/agent-chat-events.integration.test.ts'];
const output = path.join(root, 'docs/renderer-performance/acceptance/f4-send-preparation.json');
...
function validate(report) {
  assert.equal(report.kind, 'agent_send_preparation_acceptance'); assert.equal(report.status, 'passed');
  assert.deepEqual(report.suites, suites); assert.equal(report.tests.length, 44);
  assert(report.tests.every(test => test.status === 'passed' && test.durationMs >= 0));
  assert.equal(new Set(report.tests.map(test => test.name)).size, 44);
  ...
  assert.equal(report.baseline.tests.length, 22);
  assert.equal(report.baseline.tests.filter(test => test.status === 'failed').length, 20);
  const successfulBaseline = report.baseline.tests.filter(test => test.status === 'passed').map(test => test.title).sort();
  assert.deepEqual(successfulBaseline, ['keeps accepted context in the runtime prompt and the author text in the displayed transcript',
    'preserves an already-submitted turn after stopAfterTool while its start acknowledgement is pending'].sort());
  ...
  assert.deepEqual(report.acceptance, { productionStore: true, repositories: 'synthetic-ports', provider: 'synthetic-transport',
    native: 'not-run', performanceBudget: 'not-evaluated', workingMemoryStorageCancellation: 'not-proven' });
}
if (process.argv.includes('--check')) {
  const report = JSON.parse(readFileSync(output, 'utf8')); validate(report);
  assert.equal(report.source.fingerprint, fingerprint(), 'Send preparation evidence is stale.');
  console.log('Agent send ownership evidence matches source: 44 checks; native and provider acceptance not inferred.');
} else {
  const temporary = mkdtempSync(path.join(tmpdir(), 'drifting-agent-send-acceptance-'));
  ...
  try {
    ...
    const report = { kind: 'agent_send_preparation_acceptance', status: 'passed', generatedAt: new Date().toISOString(),
      ...
      acceptance: { productionStore: true, repositories: 'synthetic-ports', provider: 'synthetic-transport', native: 'not-run',
        performanceBudget: 'not-evaluated', workingMemoryStorageCancellation: 'not-proven' } };
    validate(report); writeFileSync(output, JSON.stringify(report, null, 2) + '\n');
    console.log('Agent send acceptance: identical 22 store probes, baseline 20 failures -> 0; 44 current checks passed.');
  } finally {

...
// tables: element (id, project_id, category_id, type, name, content_json, summary_json, created_at, updated_at)
//         element_category (id, name, description_json, color)
//         element_tag (id, element_id, name, created_at)
...
export function createBookElementSqliteRepository(projectId: string): BookElementRepository {
    return {
        async findById(id: string): Promise<ElementRecord | null> {
            const rows = await query<ElementRecord>(`SELECT * FROM element WHERE id='${esc(id)}'`);
            return rows[0] ?? null;
        },
        async findAll(): Promise<ElementRecord[]> {
            return query<ElementRecord>(`SELECT * FROM element WHERE project_id='${esc(projectId)}'`);
        },
        async findAllByProject(pid: string): Promise<ElementRecord[]> {
            return query<ElementRecord>(`SELECT * FROM element WHERE project_id='${esc(pid)}'`);
        },
        async findAllByCategory(pid: string, category: string): Promise<ElementRecord[]> {
            ...
            return query<ElementRecord>(
                `SELECT e.* FROM element e JOIN element_category c ON e.category_id = c.id
         WHERE e.project_id='${esc(pid)}' AND c.name='${esc(category)}'`
            );
        },
        async findAllByTag(pid: string, tag: string): Promise<ElementRecord[]> {
            return query<ElementRecord>(
                `SELECT e.* FROM element e JOIN element_tag t ON e.id = t.element_id
        ...
        async create(data: Partial<ElementRecord>): Promise<ElementRecord> {
            ...
            await run(`INSERT INTO element (id, project_id, category_id, type, name, content_json, summary_json, created_at, updated_at)
        ...
        async ensureCategory(category: ElementCategoryRecord): Promise<void> {
            await run(`INSERT OR IGNORE INTO element_category (id, name, description_json, color)
        VALUES ('${esc(category.id)}','${esc(category.name)}','${esc(category.description_json)}',${category.color ? `'${esc(category.color)}'` : 'NULL'})`);
        },
        async setElementCategory(elementId: string, categoryId: string): Promise<void> {
        ...
        async getElementCategory(elementId: string): Promise<string | null> {
            const rows = await query<{ category_id: string }>(`SELECT category_id FROM element WHERE id='${esc(elementId)}'`);
            return rows[0]?.category_id ?? null;
        },
        async updateElementCategory(elementId: string, categoryId: string): Promise<void> {
            await run(`UPDATE element SET category_id='${esc(categoryId)}' WHERE id='${esc(elementId)}'`);
        },
        async getElementTags(elementId: string): Promise<string[]> {
            const rows = await query<{ name: string }>(`SELECT name FROM element_tag WHERE element_id='${esc(elementId)}'`);
            return rows.map(r => r.name);
        },
        async addElementTag(elementId: string, tag: string): Promise<void> {
            await run(`INSERT OR IGNORE INTO element_tag (id, element_id, name, created_at)
        VALUES ('${crypto.randomUUID()}','${esc(elementId)}','${esc(tag)}','${new Date().toISOString()}')`);
        },
        ...
        async setElementTags(elementId: string, tags: string[]): Promise<void> {
            ...
            for (const t of tags) {
                await run(`INSERT INTO element_tag (id, element_id, name, created_at)
          VALUES ('${crypto.randomUUID()}','${esc(elementId)}','${esc(t)}','${new Date().toISOString()}')`);
            }
        },
        async getElementContent(elementId: string): Promise<string> {
            const row = await this.findById(elementId);
            return row?.content_json ?? '{}';
        },
        async setElementContent(elementId: string, content: string): Promise<void> {
            await run(`UPDATE element SET content_json='${esc(content)}', updated_at='${esc(new Date().toISOString())}' WHERE id='${esc(elementId)}'`);
        }
    };
}
...
export function createCategorySqliteRepository(): BookElementCategoryRepository {
    return {
        async findAll(): Promise<string[]> {
            const rows = await query<{ name: string }>(`SELECT name FROM element_category`);
            return rows.map(r => r.name);
        },
        async findByName(name: string): Promise<ElementCategoryRecord | null> {
            const rows = await query<ElementCategoryRecord>(`SELECT * FROM element_category WHERE name='${esc(name)}'`);
            return rows[0] ?? null;
        },
        async create(name: string, color?: string): Promise<ElementCategoryRecord> {
            ...
            await run(`INSERT INTO element_category (id, name, description_json, color)
        ...
        async update(name: string, color: string): Promise<ElementCategoryRecord | null> {
            await run(`UPDATE element_category SET color='${esc(color)}' WHERE name='${esc(name)}'`);
            return this.findByName(name);
        },
        async delete(name: string): Promise<boolean> {
            await run(`DELETE FROM element_category WHERE name='${esc(name)}'`);
            return true;
        },
        async ensureCategory(name: string): Promise<void> {
            await run(`INSERT OR IGNORE INTO element_category (id, name, description_json) VALUES ('${crypto.randomUUID()}','${esc(name)}','{}')`);
        }
    };
}

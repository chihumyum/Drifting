...
import {
  createElementTagRepository,
  createElementTagLinkRepository,
} from '../sqlite-repo/element-tag-repo';
import { initDatabase, getDb } from '../lib/db';
import type { ElementTag } from '../domain/element-tag';
import { useDataStore } from '../store/data-store';
import { withOptimisticUpdate } from './optimistic';
import {
  syncElementTagCreate,
  syncElementTagDelete,
  syncElementTagLinkCreate,
  syncElementTagLinkDelete,
  syncElementTagsSet,
} from './sync-helpers';
...
export function useElementTag({ projectId, userId }: UseElementTagContext) {
  ...
  const updateElementTagIdsInStore = useCallback(
    (elementId: string, tagIds: string[], updatedAt?: string) => {
  ...
  const createTag = useCallback(
    async (input: CreateElementTagInput) => {
  ...
  const getTagsForElement = useCallback(
    async (elementId: string, projectId?: string) => {
      await ensureDb(projectId);
  ...
  const getElementsWithTag = useCallback(
  ...
  const addTagToElement = useCallback(
    async (elementId: string, tagId: string, projectId?: string) => {
      const pid = await ensureDb(projectId);
      const db = getDb();
  ...
  const removeTagFromElement = useCallback(
    async (elementId: string, tagId: string, projectId?: string) => {
      const pid = await ensureDb(projectId);
      const db = getDb();
      const updatedAt = new Date().toISOString();
  ...
  const setElementTags = useCallback(
    async (elementId: string, tagIds: string[], projectId?: string) => {
      const pid = await ensureDb(projectId);
      const db = getDb();
      const updatedAt = new Date().toISOString();
  ...
  const createAndAddTagToElement = useCallback(
    async (elementId: string, input: CreateElementTagInput) => {
      const projectId = await ensureDb(input.projectId);
      const db = getDb();
      ...
      return withOptimisticUpdate({
        ...
        onSuccess: (tag) => {
          if (!tag) return;
          const latestCurrent = useDataStore
            .getState()
            .bookElements.find((el) => el.id === elementId);

          const nextTagIds = latestCurrent
            ? latestCurrent.tagIds.includes(tag.id)
              ? latestCurrent.tagIds
              : [...latestCurrent.tagIds, tag.id]
            : null;

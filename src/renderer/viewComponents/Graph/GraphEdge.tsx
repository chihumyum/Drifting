...
export const GraphEdge = memo(
  ({
    ...
    const sourcePoint = getPointOnBorder(
      sourcePos.x,
      sourcePos.y,
      targetPos.x,
      targetPos.y,
    ...
    const targetPoint = getPointOnBorder(
      targetPos.x,
      targetPos.y,
      sourcePos.x,
      sourcePos.y,
    ...
    return (
      ...
      >
        ...
        <path
          ...
          markerEnd={
            isDirected
              ? `url(#${customMarkerId || (isSelected ? 'arrowhead-selected' : 'arrowhead-default')})`
              : undefined
          }
          className="transition-colors duration-200"
          stroke={
            isStorylineEdge
              ? (style?.stroke as string) || '#94a3b8'
              : isSelected
                ? '#3b82f6'
                : '#94a3b8'
          }
        ...
        {edge.label && !edge.id.startsWith('storyline-') && (
          ...
          >
            <div className="flex justify-center items-center">
              <span
                className={`px-2 py-0.5 text-[10px] rounded-full border shadow-sm truncate max-w-full ${isSelected ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-gray-200 text-gray-500'}`}
              >
                {edge.label}
              </span>
            </div>

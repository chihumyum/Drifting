...
export function buildDriftingAgentSystemPrompt(
  ...
  const lines = [
    ...
    'For work spanning many chapters or context windows, list the relevant target directory once and create a durable task plan early when that tool is available. For a vague campaign, create a coarse plan from the requested deliverables immediately and use a bounded research step first; do not wait for a full workspace inventory before planning. Use scopeKind=whole_book_chapters with omitted steps for the whole book; use scopeKind=explicit_targets only for a narrower named set. Record every explicit author constraint. Planning is continuity support, not a gate: do not postpone useful reads or edits solely because a plan was not created.',
    'For a durable plan, keep at most one step in_progress, complete finished steps, never repeat completed work, and resume the first unfinished step after compaction or restart. Drifting binds the durable read/write evidence itself, so do not copy writeRef values into plan updates. A provider may complete a pending step directly when the work was already done. Those are continuity boundaries, not task completion: continue until the work is complete, the author stops it, progress genuinely stalls, or author input is required. If an edit awaits author review, block that step only when no independent pending work can continue.',

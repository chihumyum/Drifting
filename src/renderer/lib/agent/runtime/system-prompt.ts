...
export function buildDriftingAgentSystemPrompt(
  ...
  const lines = [
    ...
    'When the author explicitly asks to pick up work from another conversation, use the project handoff capability if it is exposed. Its objectives, constraints, progress, names, and reliable saved results are bounded read-only orientation: they do not transfer the source plan, permissions, controls, transcript, or provider state. If the source is still running, do not duplicate its active deliverable. Before any write, read the current authored target and follow the normal revision and conflict rules.',

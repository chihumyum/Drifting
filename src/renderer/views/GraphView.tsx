...
export function GraphView() {
  ...
  const [summaryEditor, setSummaryEditor] = useState<{ nodeId: string; summary: string } | null>(
    null,
  );
  ...
  const getAnchorOnBorder = (
    _nodePos: { x: number; y: number },
  ...
  useEffect(() => {
    ...
    bookNodes.forEach((node) => {
      ...
      if (simNode && node.position) {
        ...
        if (
          Math.abs(simNode.x - node.position.x) > 1 ||
          Math.abs(simNode.y - node.position.y) > 1
        ) {
          simNode.x = node.position.x;
          simNode.y = node.position.y;
          simNode.fx =
            dragState?.type === 'node' && dragState.id === node.id ? node.position.x : null;
          simNode.fy =
            dragState?.type === 'node' && dragState.id === node.id ? node.position.y : null;
        }
      }
    });
    ...
    if (dragState.type === 'viewport' && dragState.initialViewport) {
      ...
      if (simulationRef.current) {
        const simNode = simulationRef.current
          .nodes()
          .find((n: any) => n.id === dragState.id) as any;
        if (simNode) {
          simNode.fx = newPos.x;
          simNode.fy = newPos.y;
        }
      ...
      if (edge && tempEdgeEnd) {
        ...
        if (targetNode) {
          ...
          const originalNodeId =
            dragState.anchorType === 'source' ? edge.sourceNodeId : edge.targetNodeId;
          ...
          if (originalNode?.position) {
            ...
            if (dragState.anchorType === 'source') {
              ...
              nodeUsecases.updateEdge(dragState.id, {
  ...
  const handleControlMouseDown = (e: React.MouseEvent, edgeId: string) => {
    ...
    setDragState({
      ...
      startX: e.clientX,
      startY: e.clientY,
      initialControlPoint: edge.controlPointOffset
        ...
        <div className="absolute bottom-4 left-4 z-50 w-96 bg-white/95 backdrop-blur-sm rounded-lg shadow-xl border border-gray-200 p-4">
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button
              ...
              onClick={async () => {
                if (summaryEditor) {
                  await nodeUsecases.updateNode(summaryEditor.nodeId, {
                    summary: summaryEditor.summary,
                  });
        ...
        <svg id="graph-bg" className="absolute top-0 left-0 w-full h-full overflow-visible">
          ...
          {derivedEdges.map((edge) => {
            ...
            const sPos = source?.position
              ? { x: source.position.x ?? 0, y: source.position.y ?? 0 }
              : null;
            const tPos = target?.position
              ? { x: target.position.x ?? 0, y: target.position.y ?? 0 }
              : null;
        ...
        {bookNodes.map((node) => (
          <div key={node.id} onMouseUp={(e) => handleNodeMouseUp(e, node.id)}>
            <GraphNode
              ...
              isHighlighted={
                highlightedStorylineId
                  ? getStorylinesForNode(node.id).some((s) => s.id === highlightedStorylineId)
                  : false
              }

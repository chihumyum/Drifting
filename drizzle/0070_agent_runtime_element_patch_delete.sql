-- Certify hard-authorized element-patch deletion with the same immutable,
-- transaction-local receipt used by create/update. A delete forward receipt
-- intentionally has no postimage; its exact preimage remains in the write
-- effect and the inverse receipt proves exact restoration.

DROP TRIGGER IF EXISTS `trg_agent_runtime_element_patch_receipt_provenance`;
--> statement-breakpoint
DROP TRIGGER IF EXISTS `trg_agent_runtime_element_patch_receipt_immutable`;
--> statement-breakpoint
DROP TABLE IF EXISTS `agent_runtime_element_patch_receipt_next`;
--> statement-breakpoint

CREATE TABLE `agent_runtime_element_patch_receipt_next` (
...
INSERT INTO `agent_runtime_element_patch_receipt_next` (
	`id`,
	`effect_id`,
	`command_id`,
	`direction`,
...
--> statement-breakpoint
DROP TABLE `agent_runtime_element_patch_receipt`;
--> statement-breakpoint
ALTER TABLE `agent_runtime_element_patch_receipt_next`
	RENAME TO `agent_runtime_element_patch_receipt`;
--> statement-breakpoint

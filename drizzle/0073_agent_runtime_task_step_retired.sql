-- Whole-book manifest reconciliation keeps removed chapter steps as durable
-- audit history. SQLite CHECK constraints require a table rebuild to add the
-- terminal `retired` state without weakening the existing scope invariants.
...
INSERT INTO `agent_runtime_task_step_next` (
	...
	`result_note`, `result_ref`, `created_at`, `updated_at`, `started_at`,
	`completed_at`
)
SELECT
	`id`, `task_id`, `project_id`, `session_id`, `ordinal`, `title`,
